### ConvLSTM interpolation with External air pollution sources (China)
...
def batch_creator_china(X, X_china, batch_size, dataset_length):
    batch_x = list()
    batch_y = list()
    ...
    for i in range(len(batch_mask)):
        ...
        batch_x.append(china_input)
        batch_y.append(X[offset : offset + timesteps])
    
    batch_x = np.asarray(batch_x)
    batch_y = np.asarray(batch_y)
    batch_ymap = np.zeros((batch_size, timesteps, grid_size))
    batch_ymap[:, :, station_map] = 1.0

    # transform from: batch, timesteps, china_timesteps, china_features => batch, china_timesteps, timesteps, china_features
    batch_x = np.transpose(batch_x, (0, 2, 1, 3))
    batch_x = batch_x.reshape((batch_size, china_timesteps, timesteps*china_features))
    batch_y = batch_y.reshape((batch_size, china_output_size))
    batch_ymap = batch_ymap.reshape((batch_size, china_output_size))

    return batch_x, batch_y, batch_ymap

def batch_creator(X, X_china, batch_size, dataset_length):
    batch_x = list()
    batch_x_china = list()
    batch_y = list()
    ...
    batch_x = batch_x.reshape((batch_size, timesteps, image_size, image_size, 1))
...
display_step = 20
is_training = True
is_training_china = False

# Network Parameters
grid_size = 1024
image_size = 32
in_channel = 1 + 1 # pollution + china
out_channel = [64]
...
china_l2 = tf.nn.l2_loss(W_output) + fc_l2

# tf china output
y_china = tf.placeholder("float", [None, china_output_size])
y_map_china = tf.placeholder("float", [None, china_output_size])

if is_training_china:
    # Loss and optimizer
    yhat = tf.multiply(china_out, y_map_china)
    # loss of training data
    loss = tf.sqrt(tf.losses.mean_squared_error(labels=y_china, predictions=yhat) * loss_ratio)

    beta = 0.01
    regularizer = china_l2
    loss = tf.reduce_mean(loss + beta * regularizer)
    tf.summary.scalar('lstm_china_loss', loss)

    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss)

# tf Graph input
x = tf.placeholder("float", [None, timesteps, image_size, image_size, 1])
# concat pollution feature with china embedded => #in_channel = 2
...
if is_training:
    ...
    regularizer = out_l2
    loss = tf.reduce_mean(loss + beta * regularizer)
    tf.summary.scalar('seq2seq_china_loss', loss)
...
model_china = "model/china_embedding.ckpt"
model_path = "model/seq2seq_china.ckpt"
...
train_writer = tf.summary.FileWriter('log/seq2seq_china_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/seq2seq_china_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    if is_training_china:
        print("Training china embedding ...")
        for step in range(1, training_steps+1):
            # Make the training batch for each step
            batch_x, batch_y, batch_ymap = batch_creator_china(X_train, X_china_train, batch_size, X_train.shape[0])

            # Run optimization
            _, train_loss, summary = sess.run([optimizer, loss, merged], feed_dict = {x_china: batch_x, y_china: batch_y, y_map_china: batch_ymap})
            #train_writer.add_summary(summary, step)

            # compute error on validate set
            batch_x, batch_y, batch_ymap = batch_creator_china(X_val, X_china_val, batch_size, X_val.shape[0])

            [validate_loss, summary] = sess.run([loss, merged], feed_dict={x_china: batch_x, y_china: batch_y, y_map_china: batch_ymap})
        ...
        save_path = saver.save(sess, model_china)
        print("Model saved in file: %s" % save_path)
        exit() # finish training china embedding
...
with tf.Session(config=session_config) as sess:
    ...
    if is_training:
        ...
        for step in range(1, training_steps+1):
            # Make the training batch for each step
            batch_x_china, batch_x, batch_y, batch_ymap = batch_creator(X_train, X_china_train, batch_size, X_train.shape[0])

            # Run optimization
            _, train_loss, summary = sess.run([optimizer, loss, merged], feed_dict = {x_china: batch_x_china, x: batch_x, y: batch_y, y_map: batch_ymap})
            #train_writer.add_summary(summary, step)

            # compute error on validate set
            batch_x_china, batch_x, batch_y, batch_ymap = batch_creator(X_val, X_china_val, batch_size, X_val.shape[0])

            [validate_loss, summary] = sess.run([loss, merged], feed_dict={x_china: batch_x_china, x: batch_x, y: batch_y, y_map: batch_ymap})
            #val_writer.add_summary(summary, step)
    ...
    for i in range(test_steps):
        batch_x_china, batch_x, batch_y, batch_ymap = batch_creator(X_test, X_china_test, batch_size, X_test.shape[0])
    ...
    output_path = "output/seq2seq_china.txt"
