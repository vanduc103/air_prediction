### Compute spRMSE for air pollution interpolation with traffic volume spatiotemporal factor
...
def batch_creator(X, X_tf, batch_size, dataset_length, s):
    batch_x = list()
    batch_y = list()
    batch_xtf = list()
...
with tf.Session(config=session_config) as sess:
    ...
    for s in range(len(station_map)):
        ...
        for i in range(test_steps):
            batch_x, batch_y, batch_ymap = batch_creator(X_test, X_tf_test, batch_size, X_test.shape[0], s)
