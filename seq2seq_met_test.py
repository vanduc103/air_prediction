### Compute spRMSE for air pollution interpolation with Meteorological spatiotemporal factor
...
def batch_creator(X, X_met, batch_size, dataset_length, s):
    batch_x = list()
    batch_y = list()
    batch_xmet = list()
...
with tf.Session(config=session_config) as sess:
    ...
    for s in range(len(station_map)):
        ...
        for i in range(test_steps):
            batch_x, batch_y, batch_ymap = batch_creator(X_test, X_met_test, batch_size, X_test.shape[0], s)
