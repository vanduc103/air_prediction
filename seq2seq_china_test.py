### Compute spRMSE of ConvLSTM interpolation with External air pollution sources (China)
...
def batch_creator(X, X_china, batch_size, dataset_length, s):
    batch_x = list()
    batch_x_china = list()
    batch_y = list()
...
with tf.Session(config=session_config) as sess:
    ...
    for s in range(len(station_map)):
        ...
        for i in range(test_steps):
            batch_x_china, batch_x, batch_y, batch_ymap = batch_creator(X_test, X_china_test, batch_size, X_test.shape[0], s)
            
            start_time = time.time()
            [out_test] = sess.run([output], feed_dict={x_china: batch_x_china, x: batch_x, y: batch_y, y_map: batch_ymap})
