### ConvLSTM for air pollution forecasting with Meteorological spatiotemporal factor
...
def batch_creator(X, X_met, batch_size, dataset_length):
    batch_x = list()
    batch_y = list()
    batch_xmet = list()
    ...
    batch_x = np.concatenate((batch_x, batch_xmet), axis=3)
...
def prediction_model(x):
    ...
    # fully connected
    conv_input = tf.reshape(states[-1][-1], [-1, image_size, image_size, out_channel[0]])
    ...
    # output layer
    W_output = tf.get_variable(name='W_output', shape=[flatten_dim, output_size],   
                    initializer=tf.contrib.layers.xavier_initializer())
    b_output = tf.Variable(tf.zeros(output_size))
    output = tf.add(tf.matmul(flatten, W_output), b_output)
...
if is_training:
    ...
    tf.summary.scalar('seq2seq_forecast_met_loss', loss)
...
model_path = "model/seq2seq_forecast_met.ckpt"

# Merge all the summaries and write them out
merged = tf.summary.merge_all()
train_writer = tf.summary.FileWriter('log/seq2seq_forecast_met_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/seq2seq_forecast_met_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    if is_training:
        for step in range(1, training_steps+1):
            ...
            batch_x, batch_y, batch_ymap = batch_creator(X_train, X_met_train, batch_size, X_train.shape[0])

            # Run optimization
            _, train_loss, summary = sess.run([train_op, loss, merged], feed_dict = {x: batch_x, y: batch_y, y_map: batch_ymap}, options=run_options)
            #train_writer.add_summary(summary, step)

            # compute error on validate set
            batch_x, batch_y, batch_ymap = batch_creator(X_val, X_met_val, batch_size, X_val.shape[0])
    ...
    for i in range(test_steps):
        batch_x, batch_y, batch_ymap = batch_creator(X_test, X_met_test, batch_size, X_test.shape[0])
    ...
    output_path = "output/seq2seq_forecast_met.txt"
