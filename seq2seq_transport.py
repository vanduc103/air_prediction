###  ConvLSTM for air pollution interpolation with traffic (volume + driving speed) spatiotemporal factor
...
def batch_creator(X, X_tf, X_spd, batch_size, dataset_length):
    batch_x = list()
    batch_y = list()
    batch_xtf = list()
    batch_xsp = list()
    ...
    batch_x = np.concatenate((batch_x, batch_xtf, batch_xsp), axis=3)
...
grid_size = 1024
image_size = 32
in_channel = 1 + 2 # pollution + traffic + speed
out_channel = [64]
...
if is_training:
    ...
    tf.summary.scalar('seq2seq_trans_loss', loss)
...
model_path = "model/seq2seq_trans.ckpt"

# Merge all the summaries and write them out
merged = tf.summary.merge_all()
train_writer = tf.summary.FileWriter('log/seq2seq_trans_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/seq2seq_trans_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    if is_training:
        for step in range(1, training_steps+1):
            ...
            batch_x, batch_y, batch_ymap = batch_creator(X_train, X_tf_train, X_spd_train, batch_size, X_train.shape[0])
            ...
            batch_x, batch_y, batch_ymap = batch_creator(X_val, X_tf_val, X_spd_val, batch_size, X_val.shape[0])
    ...
    for i in range(test_steps):
        batch_x, batch_y, batch_ymap = batch_creator(X_test, X_tf_test, X_spd_test, batch_size, X_test.shape[0])
    ...
    output_path = "output/seq2seq_trans.txt"
