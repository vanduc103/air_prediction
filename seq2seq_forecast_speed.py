### ConvLSTM for air pollution forecasting with driving Speed spatiotemporal factor
...
def batch_creator(X, X_spd, batch_size, dataset_length):
    batch_x = list()
    batch_y = list()
    batch_xspd = list()
...
if is_training:
    ...
    tf.summary.scalar('seq2seq_forecast_spd_loss', loss)
...
model_path = "model/seq2seq_forecast_spd.ckpt"

# Merge all the summaries and write them out
merged = tf.summary.merge_all()
train_writer = tf.summary.FileWriter('log/seq2seq_forecast_spd_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/seq2seq_forecast_spd_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    if is_training:
        for step in range(1, training_steps+1):
            ...
            batch_x, batch_y, batch_ymap = batch_creator(X_train, X_spd_train, batch_size, X_train.shape[0])
            ...
            batch_x, batch_y, batch_ymap = batch_creator(X_val, X_spd_val, batch_size, X_val.shape[0])
    ...
    for i in range(test_steps):
        batch_x, batch_y, batch_ymap = batch_creator(X_test, X_spd_test, batch_size, X_test.shape[0])
    ...
    output_path = "output/seq2seq_forecast_spd.txt"
