### Compute spRMSE of ConvLSTM interpolation with All spatiotemporal factors
...
def batch_creator(X, X_met, X_tf, X_spd, X_china, batch_size, dataset_length, s):
    batch_x = list()
    batch_y = list()
    batch_xmet = list()
    batch_xtf = list()
...
out_channel = [64]
china_features = 3 # 3 cities of china
...
model_path = "model/seq2seq_all.ckpt"
...
train_writer = tf.summary.FileWriter('log/seq2seq_all_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/seq2seq_all_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    for s in range(len(station_map)):
        ...
        for i in range(test_steps):
            batch_x_china, batch_x, batch_y, batch_ymap = batch_creator(X_test, X_met_test, X_tf_test, X_spd_test, X_china_test, batch_size, X_test.shape[0], s)
