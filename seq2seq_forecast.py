### ConvLSTM for air pollution forecasting
...
scaler.fit(X_train.reshape(X_train.shape[0]*X_train.shape[1],1))
X_train = scaler.transform(X_train.reshape(X_train.shape[0]*X_train.shape[1],1)).reshape(X_train.shape[0], X_train.shape[1])
X_val = scaler.transform(X_val.reshape(X_val.shape[0]*X_val.shape[1],1)).reshape(X_val.shape[0], X_val.shape[1])
X_test = scaler.transform(X_test.reshape(X_test.shape[0]*X_test.shape[1],1)).reshape(X_test.shape[0], X_test.shape[1])
...
if is_training:
    ...
    tf.summary.scalar('seq2seq_forecast_loss', loss)
...
model_path = "model/seq2seq_forecast.ckpt"

# Merge all the summaries and write them out
merged = tf.summary.merge_all()
train_writer = tf.summary.FileWriter('log/seq2seq_forecast_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/seq2seq_forecast_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    output_path = "output/seq2seq_forecast.txt"
