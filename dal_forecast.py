### Deep Air Learning forecasting reimplemented for Seoul data
...
def batch_creator_all(X, batch_size, dataset_length):
    ...
    batch_x = batch_x.reshape((batch_size, input_ae))
    batch_y = batch_y.reshape((batch_size, grid_size*pred_timesteps))
    batch_ymap = batch_ymap.reshape((batch_size, grid_size*pred_timesteps))
...
output_size = grid_size * (pred_timesteps + tem)
...
if is_training_ae:
    ...
    tf.summary.scalar('dal_forecast_loss', loss_ae)

    optimizer_ae = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss_ae)

# tf Graph output
y = tf.placeholder("float", [None, grid_size*pred_timesteps])
y_map = tf.placeholder("float", [None, grid_size*pred_timesteps])

# reshape output to shape: batch, grid_size, pred_timesteps+temporal
output_re = tf.reshape(output, [-1, grid_size, pred_timesteps+tem])
output_spa = output_re[:, :, tem:tem+pred_timesteps]

if is_training_all:
    # loss of labelled training data
    yhat = tf.multiply(tf.reshape(output_spa, [-1, grid_size*pred_timesteps]), y_map)
    loss1 = tf.sqrt(tf.losses.mean_squared_error(labels=y, predictions=yhat) * loss_ratio)

    # Spatial loss
    loss_spa = 0.0
    # reshape output_spa from batch, pred_timestep*grid_size -> batch*pred_timesteps, grid_size
    output_spa = tf.reshape(output_spa, [tf.shape(output)[0] * pred_timesteps, grid_size])
    ...
    # Temporal loss
    loss_tem = 0.0
    for i in range(0, pred_timesteps):
        output_tem = output_re[:, :, i:i+tem] # get 'tem' slices of previous hours
        output_cur = output_re[:, :, i+tem:i+tem+1] # get slice of current hour
        output_cur = tf.reshape(output_cur, [-1, grid_size])
        labels = tf.slice(output_cur, begin=[0, 0], size=[-1, 1])
        predictions = tf.slice(output_tem[:,:,0], begin=[0, 0], size=[-1, 1])
        for e in range(grid_size):
            for i in range(tem):
                if e == 0 and i == 0:
                    continue
                labels = tf.concat([ labels, tf.slice(output_cur, [0, e], [-1, 1]) ], axis=1)
    ...
    tf.summary.scalar('dal_forecast_loss', loss)
...
model_ae = "model/dal_ae_forecast.ckpt"
model_path = "model/dal_forecast.ckpt"
...
train_writer = tf.summary.FileWriter('log/dal_forecast_train', flush_secs=10)
val_writer = tf.summary.FileWriter('log/dal_forecast_val', flush_secs=10)
...
with tf.Session(config=session_config) as sess:
    ...
    if is_training_ae:
        ...
        exit() # finish training Autoencoder
...
with tf.Session(config=session_config) as sess:
    ...
    output_path = "output/dal_forecast.txt"
